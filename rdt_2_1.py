...
class RDT:
    ...
    seq_num = 0
    # latest sequence number used in a received packet
    cur_pkt = Packet(None, None)
    # boolean for if sender recieved ACK
    msg_ACK = False
    ...
    def __init__(self, role_s, server_s, port):
        ...
        if role_s == 'server':
            self.net_snd = network_2_1.NetworkLayer(role_s, server_s, port)
            self.net_rcv = network_2_1.NetworkLayer(role_s, server_s, port + 1)
        else:
            self.net_rcv = network_2_1.NetworkLayer(role_s, server_s, port)
            self.net_snd = network_2_1.NetworkLayer(role_s, server_s, port + 1)
